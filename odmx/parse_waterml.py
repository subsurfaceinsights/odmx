#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# pylint: disable=c-extension-no-member, len-as-condition, consider-using-dict-comprehension

"""
This is the waterml.common module from the ulmo library v0.8.8,
commit 2f5596d (last updated 11-7-2018, latest  in master as of 4-18-23)
https://github.com/ulmo-dev/ulmo/blob/master/ulmo/waterml/common.py

Also includes the camel_to_underscore function from ulmo.util
"""
import re
import isodate
...
def parse_site_values(content_io, namespace, query_isodate=None, methods=None):
    ...
    for (_, ele) in etree.iterparse(content_io):
        if ele.tag == namespace + "timeSeries":
            ...
            if method is None:
                if len(values_elements) > 1:
                    raise ValueError('found more than one method for '
                                     f"{variable['code']}. Need to specify "
                                     'code or "all".')
                ...
                for values_element in values_elements:
                    if values_element.find(
                            namespace + f'method[@methodID="{method}"]')\
                            is not None:
                        values = _parse_values(values_element, namespace)
                        metadata = _parse_metadata(
                            values_element, metadata_elements, namespace)
...
def _element_dict(element, exclude_children=None, prepend_attributes=True):
    ...
    element_dict = {}
    element_name = camel_to_underscore(element.tag.split('}')[-1])
...
def _element_dict_attribute_name(attribute_name, element_name,
    ...
    attribute_only = camel_to_underscore(attribute_name.split('}')[-1])
...
def _parse_metadata(values_element, metadata_elements, namespace):
    ...
    for tag, collection_name, key in metadata_elements:
        underscored_tag = camel_to_underscore(tag)
        collection = [
...
def _parse_series(series, namespace):
    ...
    for include_element in include_elements:
        ...
        if not element is None:
            name = camel_to_underscore(element.tag)
...
def _parse_site_info(site_info, namespace):
    ...
    if not timezone_info is None:
        return_dict['timezone_info'] = _parse_timezone_info(timezone_info,
                                                            namespace)
    ...
    # WaterML 1.0 notes
    notes = dict([
        (camel_to_underscore(note.attrib['title'].replace(' ', '')),
            note.text)
...
def _parse_timezone_info(timezone_info, namespace):
    ...
    uses_dst_str = timezone_info.attrib.get('siteUsesDaylightSavingsTime',
                                            "false")
...
def _parse_value(value_element):
...
def _parse_variable(variable_element, namespace):
    """
    returns a dict that represents a variable for a given etree variable
    element
    """
    return_dict = _element_dict(variable_element,
                                exclude_children=['options', 'timeScale',
                                                  'timeSupport', 'unit',
                                                  'units', 'variableCode',
                                                  'variableDescription',
                                                  'variableName'])
    ...
    statistic = variable_element.find(f"{namespace}options/{namespace}"
